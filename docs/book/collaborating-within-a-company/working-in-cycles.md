...
Over the years various organizations implemented different metric-based performance evaluations to varying degrees of failure. This ongoing ambition of trying to standardize the measurement of individual productivity unveiled an excess of metrics that do NOT work. These include an unceasing list of paradoxical items: lines of code written, lines of code deleted, story points completed in a sprint, story points pushed over to next sprint, number of bug reports closed, number of bug reports opened, ...

An inherent problem we face with metric-driven performance is that when things go well, we create less metrics to measure. When we start reviewing performance by activity, rather than progress, our team will prioritise busy work with fast turn arounds over difficult problems with long-term improvements. We want our employees to be focused on delivering quality work for our product. Not on focusing to optimize their output for internal metrics.

Some companies famously pride themselves on creating an environment of constant competitiveness, where the lowest performers of a metric are let go. That mindset is the antithesis of this book. Productivity increases when our team members take on responsibility for work beyond the immediate tasks. We prefer to own an issue, rather than hitting a deadline.

Retrospectives offer a chance for our team members to reflect on their own work, estimations, and communication, they are not a place for extracting data for performance evaluation. Performance evaluation is an emotional topic which relies heavily on empathy. 
